...
int main (void)
{
                ...
                for (int e = 0; e < verts[i].adjLen; e++) {
                        ...
                        if (edges[link].u < i) {
                                ...
                                for (int ee = 0; ee < verts[edges[link].u].adjLen; ee++) {
                                        ...
                                        for (int c = 0; c < NSF_NUM_CHANNELS; c++)
						for (int p = 0; p < SIM_MAX_LOAD-1; p++)
                                                	edges[p*NSF_NUM_EDGES+link].time[c] = edges[p*NSF_NUM_EDGES+link_cp].time[c];
                                }
                        } else {
                                for (int c = 0; c < NSF_NUM_CHANNELS; c++) {
                                        if ( !((edges[link].wave & (1<<c)) >> c) ) {
						for (int p = 0; p < SIM_MAX_LOAD-1; p++)
                                                	edges[p*NSF_NUM_EDGES+link].time[c] = -log(1-(rand()%RAND_MAX+1)/(double)((unsigned)RAND_MAX+1));
                                        }
                                }
                        }
                }
	...
	for (int i = NSF_NUM_EDGES; i < (NSF_NUM_EDGES*(SIM_MAX_LOAD-1)); i++) {
		edges[i].v = edges[i-dataset].v;
		edges[i].u = edges[i-dataset].u;
		edges[i].wave = edges[i-dataset].wave;
