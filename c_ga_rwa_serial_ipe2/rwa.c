...
#endif
                        route_next = adj[rand() % notVisited];
#ifdef DEBUG_ADDIND
                        printf ("%d\n", route_next);
#endif
...
int *selection (struct Chromosome* individual, int cross_rate)
{
        int count = 0;
        int candidate[2];
	int *selected = (int*) malloc(GA_SIZE_POP * sizeof(int));

        for (int i = 0; i < GA_SIZE_POP; i++) {
                selected[i] = 0;
                candidate[0] = rand() % GA_SIZE_POP;

                if ((rand() % 100) < cross_rate) {
                        for (int j = 0; j < 3; j++) {
                                candidate[1] = rand() % GA_SIZE_POP;
...
void crossover (struct Chromosome *individual, int *mating_pool)
{
	...
	for (int i = 0; i < GA_SIZE_POP; i++)
        ...
        for (int i = 0; i < S; i++) {
                dad = mating_pool[rand() % S];
                mom = mating_pool[rand() % S];
...
struct Chromosome *mutate (struct Vertice* verts, struct Edge* edges, struct Chromosome* ind)
{
        ...
        struct Chromosome *newInd = (struct Chromosome*) malloc(sizeof(struct Chromosome));
        int route[NSF_NUM_NODES];

        int idx = rand() % ind->length;
	//printf ("Gen sorted: %d (len: %d)\n", idx, ind->length);
#ifdef DEBUG_MUTATE
	printf ("Individual:");
...
{
        int j;
        struct Chromosome *key = (struct Chromosome*) malloc(sizeof(struct Chromosome));
